...
public partial class EmptyStorage : IStorage
{
    ...
    CrystalResult IStorage.PutAndForget(ref ulong fileId, BytePool.RentedReadOnlyMemory memoryToBeShared)
        => CrystalResult.Success;

    Task<CrystalResult> IStorage.PutAsync(ref ulong fileId, BytePool.RentedReadOnlyMemory memoryToBeShared)
        => Task.FromResult(CrystalResult.Success);
