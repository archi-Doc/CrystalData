...
internal static class HashHelper
{
    ...
    public static async Task<bool> GetFarmHashAndSaveAsync(byte[] data, ISingleFiler filer)
    {
        ...
        var result = await filer.WriteAsync(0, BytePool.RentedReadOnlyMemory.CreateFrom(hash), false).ConfigureAwait(false);
        if (result != CrystalResult.Success)
        {
            return false;
        }

        result = await filer.WriteAsync(hash.Length, BytePool.RentedReadOnlyMemory.CreateFrom(data), true).ConfigureAwait(false);
        if (result != CrystalResult.Success)
        {
            return false;
        }
