...
public class RsCoderObsolete : IDisposable
{
    public const int DefaultDataSize = 8;
    public const int DefaultCheckSize = 4;

    /// <summary>
    /// Initializes a new instance of the <see cref="RsCoderObsolete"/> class (Reed-Solomon Coder.).
    /// </summary>
    ...
    public RsCoderObsolete(
        int dataSize = DefaultDataSize,
        int checkSize = DefaultCheckSize,
        int fieldGenPoly = GaloisFieldObsolete.FieldGenPoly)
    {
        if (dataSize < 1)
        {
        ...
        if (this.TotalSize >= GaloisFieldObsolete.Max)
        {
            throw new ArgumentOutOfRangeException();
        }

        this.GaloisField = GaloisFieldObsolete.Get(fieldGenPoly);
    ...
    internal GaloisFieldObsolete GaloisField { get; }
    ...
    private void GenerateEF()
    {
        ...
        for (var y = 0; y < m; y++)
        {
            ...
            for (var x = 0; x < n; x++)
            {
                ...
                exponent += y;
                if (exponent >= GaloisFieldObsolete.Mask)
                {
                    exponent -= GaloisFieldObsolete.Mask;
                }
            }
        }
    }
...
internal class GaloisFieldObsolete
{
    public const int Max = 256;
    public const int Mask = Max - 1;
    public const int FieldGenPoly = 301; // 301 > 285 > 435

    private static readonly GaloisFieldObsolete DefaultField = new(FieldGenPoly);
    private static readonly Dictionary<int, GaloisFieldObsolete> FieldCache = new();
    private static readonly object FieldCacheLock = new();

    public static GaloisFieldObsolete Get(int fieldGenPoly)
    {
        ...
        lock (FieldCacheLock)
        {
            ...
            {
                field = new GaloisFieldObsolete(fieldGenPoly);
    ...
    private GaloisFieldObsolete(int fieldGenPoly)
    {
        var gf = new byte[Max];
