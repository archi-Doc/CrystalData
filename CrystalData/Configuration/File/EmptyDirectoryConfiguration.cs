...
/// <summary>
/// Represents a directory configuration whose operations do not access external storage.
/// </summary>
[TinyhandObject]
public partial record EmptyDirectoryConfiguration : DirectoryConfiguration
{
    ...
    public override EmptyFileConfiguration CombineFile(string file)
        => new EmptyFileConfiguration(System.IO.Path.Combine(this.Path, StorageHelper.GetPathWithoutRoot(file)));

    public override EmptyDirectoryConfiguration CombineDirectory(DirectoryConfiguration directory)
        => new EmptyDirectoryConfiguration(System.IO.Path.Combine(this.Path, StorageHelper.GetPathWithoutRoot(directory.Path)));
